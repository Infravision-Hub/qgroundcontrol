...
void QmlObjectTreeModelTest::_appendWithNodeType()
{
    ...
    QVERIFY(idx.isValid());
    QCOMPARE(model.data(idx, Qt::UserRole + 2).toString(), QStringLiteral("myType")); // NodeTypeRole
}
...
void QmlObjectTreeModelTest::_dataNodeTypeRole()
{
    ...
    QCOMPARE(model.data(idx, Qt::UserRole + 2).toString(), QStringLiteral("missionItem")); // NodeTypeRole
}
